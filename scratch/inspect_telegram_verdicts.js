...
const veredictos = rawMessages.filter(m => m.text.includes('VEREDICTO') || m.text.includes('APUESTA EVITADA'));

console.log(`TOTAL VEREDICTOS EN MESSAGES.HTML: ${veredictos.length}`);
veredictos.forEach((v, idx) => {
    console.log(`\n--- VEREDICTO #${idx + 1} [${v.date} ${v.time}] ID: ${v.id} ---`);
    console.log(v.text);
});
